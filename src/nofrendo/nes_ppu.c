...
/* ------------------------------------------------------------------ */
/*  Hooked PPU memory access                                          */
/*  PPU_MEM_PTR(x)  -> pointer to the byte                            */
/*  PPU_MEM(x)      -> l-value reference to that byte (read/write)    */
/* ------------------------------------------------------------------ */
#undef  PPU_MEM
/* Accumulate PPU dots and burn 1 CPU cycle every 3 dots (~2 PPU clocks each) */
INLINE void ppu_fetch_delay(void)
{
    static int pending_dots = 0;
    pending_dots += 2;           /* one CHR fetch = one PPU dot */
...
INLINE bool is_rendering(void)
{
    /* If your emulator counts the pre‑render line as 261 rather than -1,
       keep the ‘< 240’ test — the pre‑render line passes.                */
    return ppu_enabled() && nes_getcontextptr()->scanline < 240;
}

INLINE void inc_h_rendering(void)   /* Loopy’s “coarse X” */
{
    if ((ppu.vaddr & 0x001F) == 31) {      /* coarse X == 31 ? */
        ppu.vaddr &= ~0x001F;              /* coarse X = 0          */
        ppu.vaddr ^= 0x0400;               /* flip horizontal NT bit*/
    } else {
        ppu.vaddr += 1;                    /* coarse X++            */
    }
}
INLINE void inc_v_rendering(void)
{
    /* 1. Bump fine‑Y if it’s < 7. */
    if ((ppu.vaddr & 0x7000) != 0x7000) {     /* fine‑Y < 7 ?                */
        ppu.vaddr += 0x1000;                  /* ++fine‑Y (bits 12‑14)       */
    } else {
        /* 2. fine‑Y == 7 → reset fine‑Y and adjust coarse‑Y. */
        ppu.vaddr &= ~0x7000;                 /* fine‑Y = 0                  */

        uint16_t y = (ppu.vaddr >> 5) & 0x1F; /* coarse‑Y (bits 5‑9)         */

        if (y == 29) {                        /* bottom row of a nametable?  */
            y = 0;                            /* wrap to top…                */
            ppu.vaddr ^= 0x0800;              /* …and flip vertical NT bit   */
        } else if (y == 31) {                 /* attribute row: skip NT flip */
            y = 0;                            /* wrap to top                 */
        } else {
            y++;                              /* ordinary increment          */
        }

        ppu.vaddr = (ppu.vaddr & ~0x03E0) | (y << 5);  /* write back coarse‑Y */
    }

    /* 3. Keep vaddr inside the PPU’s 15‑bit address space (bit 15 unused). */
    ppu.vaddr &= 0x7FFF;
}
...
INLINE void ppu_notify_addr(uint16 addr)
{
    if (mapper_ppu_hook && (addr & 0x2000) == 0)
...
*/
static void ppu_setstrike(int x_abs)
{
    if (!ppu.strikeflag) {
        const uint32 CYC_PER_SCAN = 341 / 3;   /* ≈113.667 */
        uint32 cpu_now  = nes6502_getcycles(false);

        /* rewind to the current scan‑line (renderer was called one ahead) */
        uint32 base = cpu_now - CYC_PER_SCAN;

       /* exact ceil((x_abs + 2) / 3) */
        uint32 offset = (x_abs + 2 + 2) / 3;


        ppu.strikeflag  = true;
        ppu.strike_cycle = base + offset;
    }
}
...
static void ppu_oamdma(uint8 value)
{
   ...
   uint32 cycles = nes6502_getcycles(false);
   if (cycles & 1)
      nes6502_burn(514);
   else
      nes6502_burn(513);
...
uint8 ppu_read(uint32 address)
{
    ...
    /* mirror mask – all PPU regs repeat every 8 bytes up to $3FFF */
...
/*──────────────────────── $2002 – PPU_STATUS ───────────────────────*/
    case PPU_STAT:
        ...
        /* sprite‑0 strike check */
        if (ppu.strikeflag &&
            nes6502_getcycles(false) >= ppu.strike_cycle)
            value |= PPU_STATF_STRIKE;

        /* clear v‑blank flag and flip‑flop */
...
{
    /* ------------------------------------------------------------ *
     * 1.  Return the *previous* buffered byte; begin new fetch      *
     * ------------------------------------------------------------ */
    ...
    if ((ppu.bg_on || ppu.obj_on) && !ppu.vram_accessible) {
        ...
        if (addr >= 0x3000) addr -= 0x1000;      /* palette mirrors          */
        ppu.vdata_latch = PPU_MEM(addr);         /* macro triggers FETCH     */
    }

    /* ------------------------------------------------------------ *
     * 2.  Auto‑increment v                                         *
     *     • Rendering on  → diagonal step (coarse‑X + fine‑Y)      *
     *     • Rendering off → classic +1 / +32                       *
     * ------------------------------------------------------------ */
    if (ppu_enabled() && nes_getcontextptr()->scanline < 240) {
        inc_h_rendering();   /* coarse‑X++ / horizontal NT toggle     */
        inc_v_rendering();   /* fine‑Y++ / full vertical path         */
    } else {
        ppu.vaddr = (ppu.vaddr + ppu.vaddr_inc) & 0x3FFF;
    }

    /* ------------------------------------------------------------ *
     * 3.  Mapper address latch (only while rendering is off)        *
     * ------------------------------------------------------------ */
    if (!ppu_enabled())
        ppu_notify_addr(ppu.vaddr);

    break;
}


/*──────────────────────── Other registers – return latch ───────────*/
    case PPU_OAMDATA:
    case PPU_CTRL0:
    case PPU_CTRL1:
    case PPU_OAMADDR:
...
/*──────────────────────── $2000 – PPU_CTRL0 ────────────────────────*/
   case PPU_CTRL0:
      ...
      /* Update temp address bits 10‑11 */
      ppu.vaddr_latch = (ppu.vaddr_latch & ~0x0C00) | ((value & 3) << 10);
      break;

/*──────────────────────── $2001 – PPU_CTRL1 ────────────────────────*/
   case PPU_CTRL1:
      ppu.ctrl1   = value;
      ppu.obj_on  = (value & PPU_CTRL1F_OBJON)  != 0;
      ppu.bg_on   = (value & PPU_CTRL1F_BGON)   != 0;
      ppu.obj_mask= (value & PPU_CTRL1F_OBJMASK)==0;
      ppu.bg_mask = (value & PPU_CTRL1F_BGMASK)==0;
      break;
...
/*──────────────────────── $2005 – PPU_SCROLL ───────────────────────*/
   case PPU_SCROLL:
      if (!ppu.flipflop) {           /* first write */
         ppu.vaddr_latch  = (ppu.vaddr_latch & ~0x001F) | (value >> 3);
         ppu.tile_xofs    =  value & 7;
      } else {                       /* second write */
         ppu.vaddr_latch  = (ppu.vaddr_latch & ~0x73E0)
                          | ((value & 0xF8) << 2)       /* coarse Y + fine Y */
                          | ((value & 7) << 12);        /* fine Y */
      }
      /* 〈NEW〉 NO ppu_notify_addr() here – v never changes on a $2005 write */
      ppu.flipflop ^= 1;
      break;

/*──────────────────────── $2006 – PPU_VADDR ───────────────────────*/
   case PPU_VADDR:
      if (!ppu.flipflop) {           /* first write – high 6 bits */
         ppu.vaddr_latch = (ppu.vaddr_latch & 0x00FF)
                         | ((value & 0x3F) << 8);
      } else {                       /* second write – low 8 bits */
         ppu.vaddr_latch = (ppu.vaddr_latch & 0xFF00) | value;
         ppu.vaddr       =  ppu.vaddr_latch;

         if (!ppu_enabled())             /* BG+OBJ both off? */
            ppu_notify_addr(ppu.vaddr);  /* edge can happen now */
      }
      ppu.flipflop ^= 1;
      break;

/* ──────────────────────── $2007 – PPU_VDATA (write) ─────────────────── */

   case PPU_VDATA:
   {
      /* ------------------------------------------------------------ *
      * 1.  CHR‑ROM/RAM or CIRAM write                                *
      * ------------------------------------------------------------ */
      if ((ppu.vaddr & 0x3FFF) < 0x3F00)                /* < $3F00 → pattern/nametable */
      {
         if ((ppu.bg_on || ppu.obj_on) && !ppu.vram_accessible)
         {
         ...
         else
         {
               ...
               if (!ppu.vram_present && addr >= 0x3000)      /* palette mirrors */
                  addr -= 0x1000;
               PPU_MEM(addr) = value;
         }
      }
      ...
      * ------------------------------------------------------------ */
      ...
      {
         if ((ppu.vaddr & 0x0F) == 0)                          /* backdrop mirrors */
         {
               for (int i = 0; i < 8; ++i)
                  ppu.palette[i << 2] = (value & 0x3F) | BG_TRANS;
         }
         else if (ppu.vaddr & 3)                               /* ignore $XX10 etc. */
         {
      ...
      * ------------------------------------------------------------ */
      ...
      /* ------------------------------------------------------------ *
      * 4.  Mapper A12 edge only when rendering is OFF                *
      * ------------------------------------------------------------ */
      if (!ppu_enabled())
         ppu_notify_addr(ppu.vaddr);
...
{
   if (scanline == 0 && ppu.bg_base == 0x1000 && ppu.obj_base == 0x0000)
         ppu_notify_addr(0x1000);
   if (scanline < 240)
   {
